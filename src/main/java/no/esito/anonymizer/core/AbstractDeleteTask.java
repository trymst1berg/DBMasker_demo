 ...
 */
public abstract class AbstractDeleteTask extends AbstractWorkTask implements IChildren {
    ...
    private void deleteOrphans(IContext context, String table, IChildRelation[] children) throws Throwable {
        for (IChildRelation ict : children) {
            ...
            if (setNull) {
                ...
                sb.append(ConfigUtil.schemaPrefix(dt.getTable()));
            }
            ...
            if (dt instanceof NotExistsDelete) {
                ...
                sb.append(" WHERE " + ccol + " IS NOT NULL AND " + ccol + " NOT IN (SELECT DISTINCT " + pcol + " FROM "
                    + table + " WHERE " + pcol + " IS NOT NULL)");
            }
            context.getDatasource().dsExecuteUpdate(sb.toString());
    ...
    private void deleteCascade(IContext context, String table, String where, IChildRelation[] children)
        ...
        String delparent = "DELETE FROM " + ConfigUtil.schemaPrefix(table) + (where != null ? " WHERE " + where : "");
        if (columns.isEmpty()) {
            context.getDatasource().dsExecuteUpdate(delparent);
        } else {
            ...
            if (preDelete) {
                List<String[]> prows = context.getDatasource().dsReadRows(table, columns, where);
                if (!prows.isEmpty()) {
                    for (IChildRelation dt : children) {
                        if (dt instanceof CascadeDelete) {
                            for (String[] ids : prows) {
                                ...
                                if (dt.setNull()) {
                                    context.getDatasource()
                                        .dsExecuteUpdate(dt.addSetNull() + (link != null ? " WHERE " + link : ""));
                                } else {
