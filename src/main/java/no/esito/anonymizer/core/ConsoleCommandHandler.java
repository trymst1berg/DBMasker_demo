...
import no.esito.anonymizer.datasource.CsvDataSource;
import no.esito.anonymizer.datasource.SqlDataSource;
...
public class ConsoleCommandHandler {
    ...
    public void run(String[] args) throws Exception {
        ...
        String csv = (String) props.get("csv.in");
        boolean isCSV=(csv != null && !csv.isEmpty() && !csv.startsWith("<"));
        IDataSource handler=isCSV?new CsvDataSource(props):new SqlDataSource(props);
    ...
    private void printRecurse(ITask[] tasks, String prefix) {
        for (ITask task : tasks) {
            ...
            if (task instanceof ITaskGroup) {
                printRecurse(((ITaskGroup) task).getTasks(), prefix + "  ");
            }
        }
    }
    ...
    private static void ping(IContext context) {
        ...
        try {
            ...
            if(datasource==null) {
                ...
                Log.info(datasource.dsPrintConnectionInfo());
    ...
    private void emptydb(IContext context) {
        ...
        try {
            for (String table : anonymizer.getEmptyDbTables().split(",")) {
                context.getDatasource().dsExecuteUpdate("DELETE FROM " + table + "");
            }
            Log.info("Finished!");
